...
namespace GPACalculator.API.Controllers
{
    [ApiController]
    [Route("API")]
    public class GetTop3EasiestSubjectsController : ControllerBase
    {
        private readonly AppDbContext _context;
        public GetTop3EasiestSubjectsController(AppDbContext context)
        {
            _context = context;
        }

        [HttpGet("get-top-3-easiest-subjects")]
        public async Task<ActionResult<List<SubjectEntity>>> GetTop3Subjects()
        {
            ...
            for (int i = 1; i <= subjectCount; i++)
            {
                ...
                top3Subjects.Sort((s2, s1) => s1.AverageScore.CompareTo(s2.AverageScore)); 
